...
1. GHA cron запускає `generate_import.py` раз на добу. Розклад у конфізі — `0 7 * * *` (07:00 UTC = 10:00 Київ), але **GitHub Actions не гарантує точний час `schedule`** і при навантаженні відкладає запуск. Фактично крон стабільно відпрацьовує ближче до **~18:00 UTC (≈21:00 Київ)**. Тому найсвіжіші дані зʼявляються **ввечері**, не вранці — імпортувати краще ввечері або наступного ранку (дані максимум добової давнини).
